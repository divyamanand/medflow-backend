...
let StaffService = class StaffService {
    ...
    async getSpecialties(staffId) {
        const raw = await this.repo.query(`SELECT ss.id, ss."staffId", ss."specialtyId", ss."primary", sp.code, sp.name, sp.description
       FROM staff_specialty ss
       JOIN specialty sp ON sp.id = ss."specialtyId"
       WHERE ss."staffId" = $1
       ORDER BY ss."primary" DESC, sp.name ASC`, [staffId]);
        return raw.map((r) => ({
            id: r.id,
            staffId: r.staffId,
    ...
    async addSpecialty(staffId, specialtyId, primary = false) {
        console.log(staffId, specialtyId, primary);
        const specialtyExists = await this.repo.query('SELECT 1 FROM specialty WHERE id = $1 LIMIT 1', [specialtyId]);
        if (!specialtyExists.length) {
            throw new Error(`Specialty with id ${specialtyId} does not exist`);
        }
        const existing = await this.repo.query('SELECT 1 FROM staff_specialty WHERE "staffId" = $1 AND "specialtyId" = $2 LIMIT 1', [staffId, specialtyId]);
        if (existing.length) {
            throw new Error('Specialty already assigned to this staff member');
        }
        if (primary) {
            await this.repo.query('UPDATE staff_specialty SET "primary" = false WHERE "staffId" = $1', [staffId]);
        }
        const result = await this.repo.query('INSERT INTO staff_specialty ("staffId", "specialtyId", "primary") VALUES ($1, $2, $3) RETURNING id', [staffId, specialtyId, primary]);
        return this.getSpecialties(staffId);
    }
    async removeSpecialty(staffId, specialtyId) {
        const result = await this.repo.query('DELETE FROM staff_specialty WHERE "staffId" = $1 AND "specialtyId" = $2 RETURNING id', [staffId, specialtyId]);
        if (!result.length) {
